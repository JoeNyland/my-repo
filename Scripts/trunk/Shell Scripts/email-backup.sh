...
rm -v /var/log/email-backup/iCloud_Inbox.log
...
touch /tmp/email-backup/iCloud/Inbox.mbox
...
getmail --getmaildir=/home//.getmail -r icloud_Inbox || exit 1000
...
tar cvjf /mnt/backup/Email/iCloud/Inbox-`date +%F`.tar.bz2 /tmp/email-backup/iCloud/Inbox.mbox || exit 1000
...
if [ -f /mnt/backup/Email/iCloud/Inbox-`date +%F`.tar.bz2 ]; then 
		echo "The backup of 'Inbox' folder completed successfully last night." > /tmp/email-backup-report.log
		echo "Please see below for the file name..." >> /tmp/email-backup-report.log
		echo "" >> /tmp/email-backup-report.log
		ls -lah /mnt/backup/Email/iCloud/Inbox-`date +%F`.tar.bz2 >> /tmp/email-backup-report.log
		sendemail -f $FROMADDRESS -t $TOADDRESS -s $SMTPSERVER -o username=$SMTPUSERNAME -o password=$SMTPPASSWORD -u "'Inbox' on iCloud backup success for " < /tmp/email-backup-report.log
		export SUCCESS4=1
	else
		echo "The backup of the 'Inbox' folder on iCloud failed to run last night. Please verify IMAP backup log below..." > /tmp/email-backup-report.log
        echo "" >> /tmp/email-backup-report.log
		tail /var/log/email-backup/iCloud_Inbox.log >> /tmp/email-backup-report.log
		sendemail -f $FROMADDRESS -t $TOADDRESS -s $SMTPSERVER -o username=$SMTPUSERNAME -o password=$SMTPPASSWORD -u "'Inbox' on iCloud backup failure for " < /tmp/email-backup-report.log
		export SUCCESS4=0
fi
...
if [[ $SUCCESS1 == 1 ]] && [[ $SUCCESS2 == 1 ]] && [[ $SUCCESS3 == 1 ]] && [[ $SUCCESS4 == 1 ]]; then
	find /mnt/backup/Email/iCloud/ -iname *.tar.bz2 -daystart -mtime +$AGE -exec rm -fv {} \;
else
	echo $SUCCESS1 $SUCCESS2 $SUCCESS3 $SUCCESS4;
fi