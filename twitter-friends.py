...
parser.add_argument('action', choices=['import', 'export'],
                    help='Choose whether to import or export the list of friends followed')
...
parser.add_argument('--enable-notifications', dest='notifications',
                    help='When importing, use this argument to enable notifications for the new friend(s) being created',
                    action='store_true')
...
def import_friends(api, user, notifications=False):
    ...
    for friend_id in in_file().readlines():
